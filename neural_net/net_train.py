...
data_dev = np.load(train_filepath)
...
for epoch in range(total_epochs + 1, total_epochs + epochs + 1):
    ...
    train_loop(X_train, Y_train, B, xfoil_net, MSELoss_fn, optimizer, verbose = False)

    # Run the validation loop
    J_val = dev_loop(X_val, Y_val, B, xfoil_net, MSELoss_fn, verbose)
